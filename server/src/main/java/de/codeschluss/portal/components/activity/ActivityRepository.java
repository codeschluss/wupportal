...
public interface ActivityRepository extends FilteredJpaRepository<ActivityEntity, String>,
  ...
  @Query("Select distinct a from ActivityEntity a join a.schedules s " 
      + "where s.startDate > current_date "
      + "and (a.name like %?1% or a.description like %?1%)")
  ...
  @Query("Select distinct a from ActivityEntity a join a.schedules s where "
  ...
  public Optional<Page<ActivityEntity>> findByProviderIn(
      List<ProviderEntity> provider, Pageable page);
