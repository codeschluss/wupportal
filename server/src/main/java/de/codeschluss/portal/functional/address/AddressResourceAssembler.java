...
public class AddressResourceAssembler extends PagingAndSortingAssembler<AddressEntity> {
	...
	protected List<Link> createResourceLinks(AddressEntity address) {
		...
		links.add(linkTo(methodOn(AddressController.class).findOne(address.getId())).withSelfRel());
		links.add(linkTo(methodOn(AddressController.class).findSuburb(address.getId())).withRel("suburb"));
